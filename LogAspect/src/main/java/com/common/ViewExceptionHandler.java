...
@Order(Ordered.LOWEST_PRECEDENCE)//值越高，优先级越小
//@RestControllerAdvice // RestControllerAdvice返回不需要加@ResponseBody注解
public class ViewExceptionHandler {
	private final Logger logger = LoggerFactory.getLogger(ViewExceptionHandler.class);
	// 运行时异常
	@ExceptionHandler(RuntimeException.class)
	@ResponseBody
	public String runtimeExceptionHandler(RuntimeException ex) {
		logger.error("viewRuntimeException",ex);
	...
	public String nullPointerExceptionHandler(NullPointerException ex) {
		//ex.printStackTrace();
		logger.error("viewNullPointerException",ex);
	...
	public String request405(HttpRequestMethodNotSupportedException ex) {
		...
		logger.error("view405-RequestMethodNotSupported请求方法不被允许数",ex);
